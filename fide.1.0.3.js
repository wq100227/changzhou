(function(window,undefined){
	...
	//检测数据是否通过fide实例化，返回布尔值
	//Is the data detected by fide instantiation, return Boolean value
	
	var isfide = function(data){
		
		return data ? data instanceof fide : fake
	};
	...
	var fide = function(selector,context){
		
		return new fide.fn.init(selector,context)
	...
	fide[attr_fn] = fide[attr_prototype] = {
		...
		init:function(selector,context){
			...
			if(isFunction(selector)) return fide.ready(selector);
    ...
    fide[attr_fn].init[attr_prototype] = fide[attr_fn];
    ...
    fide[attr_extend] = fide[attr_fn][attr_extend] = function(){
	...
	each([attr_shift,attr_pop,attr_push,attr_splice,attr_slice,attr_unshift],function(){
		...
		fide[attr_fn][this] = Array[attr_prototype][this]
    ...
    (function(){
    	...
    	each(methods,function(i){
    		...
    		fide[attr_fn][method] = function(selector){
    	...
    	each(methods,function(i){
    		...
    		fide[attr_fn][method] = function(elements){
    			...
    			var elements = exchange ? fide(elements) : elements;
    ...
    (function(){
        ...
        var attr_events = attr_fide+'events';
        ...
        var eventsCallback = function($this,type,event,allEvents,x,y){
   			...
   			return bind(myself,function(){
   				...
   				each(type,function(){
					...
					//清除事件类型中的后缀,无论是否委托其实都只绑定了一次事件
   					...
   					if(!$this[attr_events][attr_fide+self] ){
   						...
   						$this[attr_events][attr_fide+self] = real;
   		...
   		//将移动端判断条件添加到fide属性上
   		//Add the mobile end judgment conditions to the fide properties
		...
		(function(){
			...
			var attr_animationQueue = attr_fide+attr_animation;
			...
			//创建样式列表用于向样式表中添加动画规则
	   		...
	   		var styleSheet = (function(){
				...
				if(!styleSheet){
					...
					styleSheet[attr_fide+attr_animation] = {};
			...
			var getAnimationRules = function(animationName,$this){
				...
				return each(styleSheet[attr_fide+attr_animation],function(i){
			...
			var createAnimation = function(animationName,css){
				...
				var attr_animationRules = attr_fide+attr_animation;
			...
			var parseAnimationOptions = function(style,options){
	   		...
	   		each('fade slide hide show toggle'[attr_split](pattern_space),function(i){
	   			...
	   			each(types[i] ? types[i][attr_concat](['Toggle']) : [empty],function(j){
	   				...
	   				fide[attr_fn][method] = function(options,callback,opacity){
	   		...
	   		fide[attr_fn][attr_animation] = function(css,options,callback){
    	...
    	//Add methods binding events but does not provide use event delegation parameters in this method
   		...
   		fide[attr_fn][attr_addEventListener] = function(type,callback){
    	...
    	(function(){
    		...
    		each(self = [' dbl long'[attr_split](pattern_space),('over enter out leave move'+updown)[attr_split](pattern_space),'start end move cancel'[attr_split](pattern_space),[empty][attr_concat](direction),updown[attr_split](pattern_space),'load error change scroll resize tap message'[attr_split](pattern_space)  ],function(i){
    			...
    			each(this,function(j){
    				...
    				fide[attr_fn][type = i ? typePrefix+type : type+typePrefix] = function(callback,bool){
    				...
    				if(i==1 && j ==3){
    					...
    					fide[attr_fn]['hover'] = function(overCallback,outCallback){
    		...
    		each('focus blur change'.split(' '),function(){
    			...
    			fide[attr_fn][type] = function(callback,bool){
    		...
    		each('html text val'[attr_split](pattern_space),function(i){
    			...
    			fide[attr_fn][self] = function(value){
    		...
    		each('add remove toggle'[attr_split](pattern_space),function(i){
    			...
    			fide[attr_fn][method] = function(className){
    		...
    		each(('client '+attr_offset+' scroll')[attr_split](pattern_space),function(i){
    			...
    			each((attr_width+' '+attr_height+' Left Top')[attr_split](pattern_space),function(j){
    				...
    				if(j<2 || i>1)fide[attr_fn][!i ? $this[attr_toLowerCase]() : self+$this] = function(value){
    		...
    		fide[attr_fn][attr_offset] = function(){
    		...
    		fide[attr_fn][attr_offsetParent] = function(){
    		...
    		fide[attr_fn][attr_position] = function(){
    		...
    		fide[attr_fn][attr_extend]({
				...
				slice:function(start,end){
					...
					return fide(Array[attr_prototype][attr_slice][attr_call](this,start,end))
	...
	//绑定辅助函数到fide函数上
	//Binding auxiliary function to the fide function
	...
	(function(){
		...
		var ajax = function(options){
			...
			var attr_queue = attr_fide+'queue';
			...
			var createElement = fide.fn.create;
			...
			var parseOptions = function(){
				...
				if(options[attr_dataType] == attr_jsonp){
					...
					fn =attr_fide+getTime()+parseInt(Math.random()*1000);
				...
				if(!cache)options[attr_url] += attr_and+attr_fide+'='+getTime();
		...
		each(dataTypes,function(){
			...
			fide['is'+ucword(self)] = function(data){
		...
		fide['l'+attr_trim] = ltrim;
		
		fide['r'+attr_trim] = rtrim;
		
		fide.merge = fide[attr_extend] = extend;
		
		fide.toArray = toArray;
		
		//批量遍历并绑定方法
		//Batch traversal and binding method
		
		each({camelCase:camelCase,type:type,isfide:isfide,each:each,inArray:inArray,ucword:ucword,parseJSON:parseJSON,map:map,param:param,cover:cover,callback:callback,ajax:ajax,array2Object:array2Object,toNumber:toNumber,extend:extend,isWindow:isWindow,browser:browser},function(i){
		...
		//绑定each方法用于遍历自身元素
		//
		fide[attr_fn].each = function(callback){
		...
		fide[attr_ready] = function(callback,options){
			...
			if(!isFunction(callback)) return fide;
		...
		fide[attr_fn][attr_ready] = function(callback,options){
			...
			return bind(this,function(){
				...
				fide[attr_ready][attr_call](self,callback,{element:self[attr_contentWindow] ? self[attr_contentWindow].document : self})
		...
		fide[attr_cookie] = function(name,value,time){
